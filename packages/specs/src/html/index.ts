...
export function generateDocumentation (
  ...
  if (item.description && settings.documentation !== false) {
    ...
    if (normalizedDescription) result.value += normalizedDescription.value;
  }

  if (item.references && item.references.length > 0 && settings.references !== false) {

    if (result.value.length) result.value += '\n\n';

    result.value += doesSupportMarkdown
      ? item.references.map(({ name, url }) => `[${name}](${url})`).join(' | ')
      : item.references.map(({ name, url }) => `${name}: ${url}`).join('\n');
