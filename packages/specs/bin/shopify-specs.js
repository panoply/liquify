...
export const ${n}: ${e} = ${s}`}function m(e){return e=e.replace(/&lt;/g,"<"),e=e.replace(/&gt;/g,">"),e=e.replace(/&quot;/g,'"'),e=e.replace(/&#39;/g,"'"),e=e.replace(/&amp;/g,"&"),e}function P(e,n,s){let i=[],{name:a,summary:u=null,description:t=null,examples:d,deprecated:p,deprecation_reason:c}=n;if(p===!0&&(i.push("\u26A0\uFE0F **DEPRECATED** \u26A0\uFE0F"),c!==""?i.push(m(c),"---"):i.push("No deprecation reason has been provided by the Shopify team \u{1F921}.","---")),t!==""&&u!==null){let l=m(t).replace(/>\s(Tip|Note):/g,`

**$1**
`).replace(/\(\/(docs\/api.*(?=\)))/g,"(https://shopify.dev/$1").replace(/\]\(\/(.*)(?=\))/g,"](https://shopify.dev/$1"),r=m(u).replace(/\(\/(docs\/api.*(?=\)))/g,"(https://shopify.dev/$1").replace(/\]\(\/(.*)(?=\))/g,"](https://shopify.dev/$1");l.trimStart()[0]===">"?i.push(m(r),l):i.push(m(r)+" "+l)}else{let l="";if(t!==""&&(l=m(t).replace(/>\s(Tip|Note):/g,`

**$1**
`).replace(/\(\/(docs\/api.*(?=\)))/g,"(https://shopify.dev/$1").replace(/\]\(\/(.*)(?=\))/g,"](https://shopify.dev/$1")),u.length>0){let r=m(u).replace(/\(\/(docs\/api.*(?=\)))/g,"(https://shopify.dev/$1").replace(/\]\(\/(.*)(?=\))/g,"](https://shopify.dev/$1");l.trimStart()[0]===">"?i.push(m(r),l):i.push(m(r)+" "+l)}i.push(l)}if(d.length>0){let[l]=d;l.name===""?i.push("#### Example"):i.push(`#### ${l.name}`),l.description!==""&&i.push(m(l.description)),l.raw_liquid!==""&&i.push("```liquid",m(l.raw_liquid),"```")}return i.push("---"),s?i.push(`[Shopify Liquid](https://shopify.dev/docs/api/liquid/${e}/${s}#${a})
`):i.push(`[Shopify Liquid](https://shopify.dev/docs/api/liquid/${e}/${a})
`),i.push(`Last Updated: ${_e()}`,`
`),i.join(`

`)}function je(){q(`${v} ${f.default.magenta("Shopify Filters")}`);let e=(0,S.join)(x,"node_modules/.specs/data","filters.json"),n=T.default.readFileSync(e).toString(),s=JSON.parse(n),i={},a=j.filters;for(let t of s)if(!(t.name in a))if(i[t.name]={},i[t.name].description=P("filters",t),t.return_type[0].type==="string"?i[t.name].returns="string":t.return_type[0].type==="number"?i[t.name].returns="number":t.return_type[0].type==="boolean"?i[t.name].returns="boolean":t.return_type[0].type==="array"?i[t.name].returns="array":t.return_type[0].type==="boolean"?i[t.name].returns="boolean":t.return_type[0].type==="object"?i[t.name].returns="object":i[t.name].returns="any",t.deprecated===!0&&(i[t.name].deprecated=t.deprecated),t.parameters.length===0){let d=[],p=[],c=t.syntax.split(" ");if(c[2][c[2].length-1]===":"){d.push(c[2]);let l=0;for(let r of c.slice(3)){l=l+1;let h;r.slice(0,-1)==="string"?h="string":r.slice(0,-1)==="number"?h="number":r.slice(0,-1)==="boolean"?h="boolean":r.slice(0,-1)==="array"?h="array":r.slice(0,-1)==="boolean"?h="boolean":r.slice(0,-1)==="object"?h="object":h="any",r[r.length-1]===","?(h==="string"?d.push("'$"+l+"',"):d.push("$"+l+","),p.push({type:h,required:!0})):(h==="string"?d.push("'$"+l+"'"):d.push("$"+l+","),p.push({type:h,required:!0}))}i[t.name].snippet=d.join(" "),i[t.name].arguments=p}}else{let d=t.syntax.split(" "),p=[],c=[];p.push(d[2]),d.splice(0,3);for(let l=0;l<t.parameters.length;l++){let r=t.parameters[l];if(d.length>0&&d[0][d[0].length-1]===","){if("arguments"in i[t.name]&&i[t.name].arguments.length<0)for(let h=0;h<d.length;h++){let y;r.types[0]==="string"?y="string":r.types[0]==="number"?y="number":r.types[0]==="boolean"?y="boolean":r.types[0]==="array"?y="array":r.types[0]==="boolean"?y="boolean":r.types[0]==="object"?y="object":y="any";let w=d[h],k={type:y};r.required===!0&&(k.required=!0),r.description!==""&&(k.description=m(r.description).replace(/\]\(\/(.*)(?=\))/g,"](https://shopify.dev/$1")),c.push(k),w[w.length-1]===","?y==="string"?p.push("'$"+(h+1)+"',"):p.push("$"+(h+1)+","):y==="string"?p.push("'$"+(h+1)+"'"):p.push("$"+(h+1))}}else if(Array.isArray(i[t.name].arguments)){c.length===0&&c.push({type:"parameter",value:{}}),c[c.length-1].type!=="parameter"&&c.push({type:"parameter",value:{}}),r.required&&p.push(`${r.name}:`,"${"+(l+1)+"}");let h;r.types[0]==="string"?h="string":r.types[0]==="number"?h="number":r.types[0]==="boolean"?h="boolean":r.types[0]==="array"?h="array":r.types[0]==="boolean"?h="boolean":r.types[0]==="object"?h="object":h="any",c[c.length-1].value[r.name]={type:h,required:r.required},r.description!==""&&(c[c.length-1].value[r.name].description=m(r.description).replace(/\]\(\/(.*)(?=\))/g,"](https://shopify.dev/$1"))}}p.length>0&&(p.length===1&&p[0][p[0].length-1]===":"&&(p[0]=p[0].slice(0,-1)),i[t.name].snippet=p.join(" ")),c.length>0&&(i[t.name].arguments=c)}Object.keys(i).length>0&&(q(v+" "+f.default.bold.redBright("NEW FILTERS AVAILABLE")),Object.keys(i).forEach(t=>{q(v+" "+f.default.white(t))}))}function Te(){q(`${v} ${f.default.magenta("Shopify Objects")}`);let e=(t,d)=>{let p={scope:null,items:null,type:"any",literal:null},c=t[0].type,l=t[0].array_value;c==="string"||c==="boolean"||c==="number"?p.type=c:c==="array"?(p.type=c,l==="string"||l==="boolean"||l==="number"?p.items=l:p.scope=l):c!==""&&d.some(r=>r.name===c)&&(p.type="object",p.scope=c);for(let{name:r}of t)r!==""&&(p.literal===null&&(p.literal=[]),p.literal.push(r));return p},n=(0,S.join)(x,"node_modules/.specs/data","objects.json"),s=T.default.readFileSync(n).toString(),i=JSON.parse(s),a={};for(let t of i){if(a[t.name]={summary:t.summary},t.access.global===!0&&(a[t.name].global=t.access.global),t.deprecated===!0&&(a[t.name].deprecated=t.deprecated),t.access.template.length>0&&(a[t.name].template=t.access.template),a[t.name].description=P("objects",t),t.return_type.length>0){let{type:d,scope:p,literal:c}=e(t.return_type,i);c!==null&&(a[t.name].literal=c),p!==null?(a[t.name].type=d,a[t.name].scope=p):a[t.name].type=d}if(t.properties.length>0){a[t.name].type="object",typeof a[t.name].properties!="object"&&(a[t.name].properties={});for(let d of t.properties)if(a[t.name].properties[d.name]={type:22},a[t.name].properties[d.name].description=P("objects",d,t.name),t.deprecated===!0&&(a[t.name].properties[d.name].deprecated=t.deprecated),d.return_type.length>0){let{type:p,scope:c,literal:l}=e(d.return_type,i);l!==null&&(a[t.name].properties[d.name].literal=l),c!==null?(a[t.name].properties[d.name].type=p,a[t.name].properties[d.name].scope=c):a[t.name].properties[d.name].type=p}}else a[t.name].type!=="array"&&(a[t.name].const=!0)}let u=JSON.stringify(a,null,2);q(v+f.default.cyan(` Writing objects JSON: ${f.default.whiteBright("data/liquid/shopify/objects.json")} `)),T.default.writeFileSync((0,S.join)(x,"data/liquid/shopify/objects.json"),u),q(v+f.default.cyan(` Writing objects DATA: ${f.default.whiteBright("src/liquid/data/shopify/objects.ts")} `)),T.default.writeFileSync((0,S.join)(x,"src/liquid/data/shopify/objects.ts"),Le("Objects","objects",u))}
