...
> **NOTE**
>
> To iterate over a data set and create pages for individual chunks of data, use pagination. Enable in your template\u2019s front matter by adding the pagination key.`,properties:{items:{type:"array",description:"Array of current page\u2019s chunk of data"},pageNumber:{type:"number",description:"Current page number, `0` indexed"},hrefs:{type:"array",description:"Array of all page hrefs (in order)",items:"string"},href:{type:"object",properties:{next:{type:"string",description:'Put inside `<a href="">Next Page</a>`'},prev:{type:"string",description:'Put inside `<a href="">Previous Page</a>`'},first:{type:"string"},last:{type:"string"}}},pages:{type:"array",description:"Array of all chunks of paginated data (in order)"},page:{type:"object",properties:{next:{type:"any",description:'Put inside `<a href="">Next Page</a>`'},prev:{type:"any",description:'Put inside `<a href="">Previous Page</a>`'},first:{type:"any"},last:{type:"any"}}}},reference:{name:"Eleventy Liquid",url:"https://www.11ty.dev/docs/pagination/"}}};var we=M({},ie,j),Te=M({},L);var k=process.cwd(),q=`${y.default.gray("[")}SPECS${y.default.gray("]")} ${y.default.magenta("RUN")}`,{log:b}=console;Se();function Se(){b(`${q} ${y.default.magenta.bold("Building Shopify Specifications")}`),ke(),xe(),b(`${q} ${y.default.magenta.bold("Finished Shopify Specifications")}`)}function Ue(){let e=new Date,t=e.getFullYear(),d=e.getMonth()+1,n=e.getDay(),r=i=>({1:"January",2:"February",3:"March",4:"April",5:"May",6:"June",7:"July",8:"August",9:"September",10:"October",11:"November",12:"December"})[i];return`${(i=>{let s=i%10,p=i%100;return i+(s===1&&p!==11?"st":s===2&&p!==12?"nd":s===3&&p!==13?"rd":"th")})(n)} ${r(d)} ${t}`}function Me(e,t,d){return`import { ${e} } from '../..'

export const ${t}: ${e} = ${d}`}function m(e){return e=e.replace(/&lt;/g,"<"),e=e.replace(/&gt;/g,">"),e=e.replace(/&quot;/g,'"'),e=e.replace(/&#39;/g,"'"),e=e.replace(/&amp;/g,"&"),e}function J(e,t,d){let n=[],{name:r,summary:f=null,description:i=null,examples:s,deprecated:p,deprecation_reason:c}=t;if(p===!0&&(n.push("\u26A0\uFE0F **DEPRECATED** \u26A0\uFE0F"),c!==""?n.push(m(c),"---"):n.push("No deprecation reason has been provided by the Shopify team \u{1F921}.","---")),i!==""&&f!==null){let a=m(i).replace(/>\s(Tip|Note):/g,`

**$1**
`).replace(/\(\/(docs\/api.*(?=\)))/g,"(https://shopify.dev/$1").replace(/\]\(\/(.*)(?=\))/g,"](https://shopify.dev/$1"),u=m(f).replace(/\(\/(docs\/api.*(?=\)))/g,"(https://shopify.dev/$1").replace(/\]\(\/(.*)(?=\))/g,"](https://shopify.dev/$1");a.trimStart()[0]===">"?n.push(m(u),a):n.push(m(u)+" "+a)}else{let a="";if(i!==""&&(a=m(i).replace(/>\s(Tip|Note):/g,`

**$1**
`).replace(/\(\/(docs\/api.*(?=\)))/g,"(https://shopify.dev/$1").replace(/\]\(\/(.*)(?=\))/g,"](https://shopify.dev/$1")),f.length>0){let u=m(f).replace(/\(\/(docs\/api.*(?=\)))/g,"(https://shopify.dev/$1").replace(/\]\(\/(.*)(?=\))/g,"](https://shopify.dev/$1");a.trimStart()[0]===">"?n.push(m(u),a):n.push(m(u)+" "+a)}n.push(a)}if(s.length>0){let[a]=s;a.name===""?n.push("#### Example"):n.push(`#### ${a.name}`),a.description!==""&&n.push(m(a.description)),a.raw_liquid!==""&&n.push("```liquid",m(a.raw_liquid),"```")}return n.push("---"),d?n.push(`[Shopify Liquid](https://shopify.dev/docs/api/liquid/${e}/${d}/${r})
`):n.push(`[Shopify Liquid](https://shopify.dev/docs/api/liquid/${e}/${r})
`),n.push(`Last Updated: ${Ue()}`,`
`),n.join(`

`)}function xe(){b(`${q} ${y.default.magenta("Shopify Filters")}`);let e=(0,S.join)(k,"node_modules/.specs/data","filters.json"),t=T.default.readFileSync(e).toString(),d=JSON.parse(t),n={},r=x.filters;for(let s of d)if(!(s.name in r))if(n[s.name]={},n[s.name].description=J("filters",s),s.return_type[0].type==="string"?n[s.name].returns="string":s.return_type[0].type==="number"?n[s.name].returns="number":s.return_type[0].type==="boolean"?n[s.name].returns="boolean":s.return_type[0].type==="array"?n[s.name].returns="array":s.return_type[0].type==="boolean"?n[s.name].returns="boolean":s.return_type[0].type==="object"?n[s.name].returns="object":n[s.name].returns="any",s.deprecated===!0&&(n[s.name].deprecated=s.deprecated),s.parameters.length===0){let p=[],c=[],a=s.syntax.split(" ");if(a[2][a[2].length-1]===":"){p.push(a[2]);let u=0;for(let l of a.slice(3)){u=u+1;let h;l.slice(0,-1)==="string"?h="string":l.slice(0,-1)==="number"?h="number":l.slice(0,-1)==="boolean"?h="boolean":l.slice(0,-1)==="array"?h="array":l.slice(0,-1)==="boolean"?h="boolean":l.slice(0,-1)==="object"?h="object":h="any",l[l.length-1]===","?(h==="string"?p.push("'$"+u+"',"):p.push("$"+u+","),c.push({type:h,required:!0})):(h==="string"?p.push("'$"+u+"'"):p.push("$"+u+","),c.push({type:h,required:!0}))}n[s.name].snippet=p.join(" "),n[s.name].arguments=c}}else{let p=s.syntax.split(" "),c=[],a=[];c.push(p[2]),p.splice(0,3);for(let u=0;u<s.parameters.length;u++){let l=s.parameters[u];if(p.length>0&&p[0][p[0].length-1]===","){if("arguments"in n[s.name]&&n[s.name].arguments.length<0)for(let h=0;h<p.length;h++){let g;l.types[0]==="string"?g="string":l.types[0]==="number"?g="number":l.types[0]==="boolean"?g="boolean":l.types[0]==="array"?g="array":l.types[0]==="boolean"?g="boolean":l.types[0]==="object"?g="object":g="any";let R=p[h],U={type:g};l.required===!0&&(U.required=!0),l.description!==""&&(U.description=m(l.description).replace(/\]\(\/(.*)(?=\))/g,"](https://shopify.dev/$1")),a.push(U),R[R.length-1]===","?g==="string"?c.push("'$"+(h+1)+"',"):c.push("$"+(h+1)+","):g==="string"?c.push("'$"+(h+1)+"'"):c.push("$"+(h+1))}}else if(Array.isArray(n[s.name].arguments)){a.length===0&&a.push({type:"parameter",value:{}}),a[a.length-1].type!=="parameter"&&a.push({type:"parameter",value:{}}),l.required&&c.push(`${l.name}:`,"${"+(u+1)+"}");let h;l.types[0]==="string"?h="string":l.types[0]==="number"?h="number":l.types[0]==="boolean"?h="boolean":l.types[0]==="array"?h="array":l.types[0]==="boolean"?h="boolean":l.types[0]==="object"?h="object":h="any",a[a.length-1].value[l.name]={type:h,required:l.required},l.description!==""&&(a[a.length-1].value[l.name].description=m(l.description).replace(/\]\(\/(.*)(?=\))/g,"](https://shopify.dev/$1"))}}c.length>0&&(c.length===1&&c[0][c[0].length-1]===":"&&(c[0]=c[0].slice(0,-1)),n[s.name].snippet=c.join(" ")),a.length>0&&(n[s.name].arguments=a)}let f=JSON.stringify(n,function(s,p){return p&&p.exec===RegExp.prototype.exec?new RegExp(p).source:p},2);Object.keys(n).length>0&&(b(q+" "+y.default.bold.redBright("NEW FILTERS AVAILABLE")),b(q+y.default.cyan(` Writing filters JSON: ${y.default.whiteBright("data/liquid/shopify/filters.json")} `)),T.default.writeFileSync((0,S.join)(k,"data/liquid/shopify/filters.json"),f),Object.keys(n).forEach(s=>{b(q+" "+y.default.white(s))}))}function ke(){b(`${q} ${y.default.magenta("Shopify Objects")}`);let e=(i,s)=>{let p={scope:null,items:null,type:"any",literal:null},c=i[0].type,a=i[0].array_value;c==="string"||c==="boolean"||c==="number"?p.type=c:c==="array"?(p.type=c,a==="string"||a==="boolean"||a==="number"?p.items=a:p.scope=a):c!==""&&s.some(u=>u.name===c)&&(p.type="object",p.scope=c);for(let{name:u}of i)u!==""&&(p.literal===null&&(p.literal=[]),p.literal.push(u));return p},t=(0,S.join)(k,"node_modules/.specs/data","objects.json"),d=T.default.readFileSync(t).toString(),n=JSON.parse(d),r={};for(let i of n){if(r[i.name]={summary:i.summary},i.access.global===!0&&(r[i.name].global=i.access.global),i.deprecated===!0&&(r[i.name].deprecated=i.deprecated),i.access.template.length>0&&(r[i.name].template=i.access.template),r[i.name].description=J("objects",i),i.return_type.length>0){let{type:s,scope:p,literal:c}=e(i.return_type,n);c!==null&&(r[i.name].literal=c),p!==null?(r[i.name].type=s,r[i.name].scope=p):r[i.name].type=s}if(i.properties.length>0){r[i.name].type="object",typeof r[i.name].properties!="object"&&(r[i.name].properties={});for(let s of i.properties)if(b(y.default.gray(`  - ${s.name}`)),r[i.name].properties[s.name]={type:22},r[i.name].properties[s.name].description=J("objects",s,i.name),i.deprecated===!0&&(r[i.name].properties[s.name].deprecated=i.deprecated),s.return_type.length>0){let{type:p,scope:c,literal:a}=e(s.return_type,n);a!==null&&(r[i.name].properties[s.name].literal=a),c!==null?(r[i.name].properties[s.name].type=p,r[i.name].properties[s.name].scope=c):r[i.name].properties[s.name].type=p}}else r[i.name].type!=="array"&&(r[i.name].const=!0)}let f=JSON.stringify(r,null,2);b(q+y.default.cyan(` Writing objects JSON: ${y.default.whiteBright("data/liquid/shopify/objects.json")} `)),T.default.writeFileSync((0,S.join)(k,"data/liquid/shopify/objects.json"),f),b(q+y.default.cyan(` Writing objects DATA: ${y.default.whiteBright("src/liquid/data/shopify/objects.ts")} `)),T.default.writeFileSync((0,S.join)(k,"src/liquid/data/shopify/objects.ts"),Me("Objects","objects",f))}
