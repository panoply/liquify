...
export default (function () {
  ...
  const syntactic =
    ...
    ((hierarch) => {
      return {
         ...
         */
        get parenNode() {
          return hierarch[hierarch.length - 1];
        },
         ...
         */
        get parentName() {
          return hierarch[hierarch.length - 2];
        },
  ...
  const scan = (offset = 0) => {
    ...
    return state === ScanState.CharSeq ? CharSeq() : Scanner();
  };
  ...
  function Scanner() {
    ...
    switch (state) {
        ...
        if (s.IfRegExp(r.ObjectNameAlpha)) {
          ...
          if (spec.filter.accept("reference")) {
            ...
            if (
              ...
              s.IfNextRegExp(r.ObjectHasProperty)
            ) {
              // Next call we will look for a property notation
              state = ScanState.Object;
              return TokenType.Object;
      ...
      case ScanState.ParseError: {
        ...
        if (s.ConsumeUnless(/-?[%}]\}/, /\{[{%]/)) {
          state = ScanState.TagClose;
          return cache === ScanCache.GotoEnd ? Scanner() : TokenType.ParseError;
        }
