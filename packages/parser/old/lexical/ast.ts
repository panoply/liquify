...
export default class IAST {
  ...
  get parentNode(): Parser.ASTNode {
    return this.nodes[this.node.parent];
  }

  public getNodeAt(position: Position | number, updateNode: boolean = true) {
    const offset =
  ...
  public getTokenAt(position: Position | number, updateNode: boolean = true) {
    const offset =
      typeof position === "number" ? position : this.offsetAt(position);

    if (inRange(offset, this.node.offsets[0], this.node.offsets[1]))
      return this.nodes;

    const index = this.nodes.findIndex(({ offsets }) =>
      inRange(offset, offsets[0], offsets[1])
    );
  ...
  getEmbedAt(position: Position | number, updateNode: boolean = true) {
  ...
  public getParentNode(node = this.node) {
    return node.parent === node.index && node?.root === 0
      ? false
      : this.nodes[node.parent];
  }

  public getNodes(indexes: number[]) {
  ...
  public getEmbeds(languages: string[] = undefined) {
    if (this.embeds.length === 0) return false;
