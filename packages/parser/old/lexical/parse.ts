...
import ASTNode from "lexical/node";
...
export function parse(
   ...
   *
   */
  const reset = (context: number = NaN): void => {
    // RESET LETTINGS
    state = undefined;
    node = undefined;
  ...
  while (token !== TokenType.EOS) {
    switch (token) {
      ...
      case TokenType.Whitespace:
        if (Config.context && Config.whitespace)
          context.add(TokenContext.Whitespace);
        break;
      case TokenType.Newline:
        if (Config.context && Config.newlines)
          context.add(TokenContext.Newline);
        break;
