import { CharCode } from '../lexical/characters';
...
export function ComputeLineOffsets (
  ...
  for (let i = 0; i < length; i++) {
    ...
    if (ch === CharCode.CAR || ch === CharCode.NWL) {
      if (ch === CharCode.CAR && i + 1 < length && text.charCodeAt(i + 1) === CharCode.NWL) i++;
 ...
 */
export function GetCodeChar (at: number = offset): number {
...
export function SkipQuotedString (consume?: number | number[] | true): boolean {
  ...
  if (GetCodeChar(location - 1) === CharCode.BWS) return SkipQuotedString(location);
...
function WhileChar (condition: Function): boolean {
  ...
  while (offset < size && condition(source.charCodeAt(offset))) {
    offset++;

    if (IsCodeChar(CharCode.NWL) || IsCodeChar(CharCode.LFD) || IsCodeChar(CharCode.CAR)) {
      line = offset;
      console.log(JSON.stringify(GetChar(offset)));
    }
  }
...
export function ReverseWhitespace (from?: number): boolean {
  ...
  while (rev--) {
    if (
      str.charCodeAt(rev) !== CharCode.WSP ||
      str.charCodeAt(rev) !== CharCode.TAB ||
      str.charCodeAt(rev) !== CharCode.NWL ||
      str.charCodeAt(rev) !== CharCode.LFD
    ) {
      break;
    }
  }
...
export function SkipWhitespace (): boolean {
  return WhileChar(
    (charCode: number) =>
      charCode === CharCode.WSP ||
      charCode === CharCode.TAB ||
      charCode === CharCode.NWL ||
      charCode === CharCode.LFD ||
      charCode === CharCode.CAR
  );
}
...
export function Whitespace (): boolean {
  ...
  return WhileChar((charCode: number) => charCode === CharCode.WSP || charCode === CharCode.TAB);
}
...
export function Newlines (): boolean {
  ...
  return WhileChar((charCode: number) => (
    charCode === CharCode.NWL ||
    charCode === CharCode.LFD ||
    charCode === CharCode.CAR
  ));
