import type { TextEdit, Diagnostic } from 'vscode-languageserver-types';
...
export class AST {
   ...
   * @todo
   */
  readonly file: { type: 'snippet' | 'section' | 'template'; };
   ...
   * Global scopes
   */
  public scopes: { [name: string]: Node } = {};
   ...
   */
  public warning (warning: ParseError) {

    if (validate(warning)) {
      const diagnostic: IDiagnostic = assign({}, Diagnostics[warning]);
      diagnostic.range = this.getRange();
  ...
  public report (error: ParseError): (location?: Range | undefined) => void {
    ...
    const diagnostic: IDiagnostic = assign({}, Diagnostics[error]);
    ...
    return (location?: Range | undefined) => {
      ...
      diagnostic.range = location === undefined ? this.getRange() : location;
   ...
   * The error diagnostics consumed by LSP
   */
  public diagnostics (): PublishDiagnosticsParams {

    return {
      uri: this.uri,
      version: this.version,
      diagnostics: this.errors.length === 0
        ? []
        : this.errors as Diagnostic[]
    };
    ...
    for (const change of edits) {
      ...
      if (endline - startline === linecount) {
        ...
        if (linecount < 10000) {
          ...
          this.lines = lines = lines.slice(0, startline + 1).concat(newlines, lines.slice(endline + 1));
        }
      }
      ...
      if (diff !== 0) {
        for (let i = startline + 1 + linecount, s = lines.length; i < s; i++) {
