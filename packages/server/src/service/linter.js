...
import controlValidation from './validations/control'
/**
 * Object Validation
 *
 * @param {string} ASTNode
 * @param {import('types/specification').Object} spec
 */
function objectValidation (ASTNode, spec, diagnostics) {

  // console.log('object', ASTNode)
 ...
 * @param {object} spec
 */
function validate (ASTNode, document, spec) {

  console.log(ASTNode, document, spec)
  return
  const { diagnostics, textDocument } = document
  const { name, offset, tag } = ASTNode
  ...
  function tagName (token) {
    ...
    if (spec.type === 'object') return objectValidation(token, spec)
...
export default (document) => (ASTNode, spec) => {
  ...
  if (ASTNode.type === TokenType.object) {
    // objectValidation(ASTNode, document, spec)
  } else if (ASTNode.type === TokenType.control) {
    controlValidation(ASTNode, document, spec)
  }
