...
export default function ({
  ...
  return (function walk (token) {
    ...
    const cond = /.*?(?=[!=<>]+|\b(?:and|or)\b|\s*-?%})/s.exec(token)
    const name = /[a-zA-Z0-9[\]-_."']/.exec(cond[0])

    if (!name) {
      return diagnostics.push({
        severity: DiagnosticSeverity.Error,
        source: 'liquify',
        message: `Invalid condition was expressed at: ${cond[0]}`,
        range: [ start, end ]
      })
    }
    ...
    compare = name[0]
    string = token.slice(cond[0].indexOf(compare) + compare.length)

    const operators = /[!=<>]+|\bor\b|\band\b/.exec(string)

    if (operators) {

      const chars = /(?:[<>]|<=|>=|==|!=|\bor\b|\band\b)/.exec(string)
      ...
      diagnostics.push({
        severity: DiagnosticSeverity.Error,
        message: `Invalid operator sequence at: ${cond[0].trim()}`,
        source: 'liquify',
