...
export default (document, contentChanges) => {
  ...
  for (const change of contentChanges) {
    ...
    const nodes = mapNodes(change)

    document.ast = document.ast.map(nodes)

    /* (
      start > node.offset[0] && end > node.offset[node.offset.length - 1]
    ) ? (
        SkipNode(node, index, start)
      ) : removed > 0 ? (
        RemoveNode(node, start, end)
      ) : changed > 0 ? (
        UpdateNode(node)
      ) : node
    ) */

    document.ast = document.ast.map((node, i) => {
      ...
      if (text.length > 0) {
        ...
        console.log(`text added ${i} of`, offset, name)
