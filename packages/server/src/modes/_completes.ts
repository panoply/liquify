...
import { Characters, INode } from '@liquify/liquid-parser';
import { html } from '@liquify/liquid-language-specs';

export function getAttributes (node: INode) {

  if (!html.tags[node.tag].attributes.length) {
    return html.attributes;
  } else {
    return html.tags[node.tag].attributes;
  }
...
export function getTags (document, position, offset, trigger) {
  ...
  return Parser.spec.entries.tags.map(
    ([
      label,
      {
        description,
...
export async function getObjectCompletion (document, offset) {
  ...
  if (isArray(record) && record.length === 1) {
    const { properties } = Parser.spec.variant.objects[record[0]];
  ...
  if (typeof record === 'number') {
    ...
    }(objects.slice(1), Parser.spec.variant.objects[objects[0]].properties));
  }
