...
export default (function markup_init () {
  ...
  function markup (source) {
    ...
    const { attributeSortList } = options.lexer_options.markup
    ...
    function recordPush (target, record, structure) {
      ...
      if (options.lexer_options.markup.parseSpace === true) {
        record.lines =
          0
      }
    ...
    function tag (end) {
      ...
      function attributeRecord () {
        ...
        const qc =
            options.lexer_options.markup.quoteConvert === undefined
              ? 'none'
              : options.lexer_options.markup.quoteConvert
        ...
        // sort the attributes
        if (
          (options.lexer_options.markup.attributeSort === true ||
          ...
          do {
            ...
            if (
              ...
              if (options.language === 'jsx' && /^(\s*\{)/.test(slice) ===
                ...
                record.begin = parse.structure[parse.structure.length - 1]
                  [1]
                record.stack = parse.structure[parse.structure.length - 1]
                  [0]
                record.token = '}'
      ...
      (function lexer_markup_tag_types () {
        ...
        if (options.lexer_options.markup.preserveAttribute === true) {
          preserve = true
        }
              ...
              if (stest === true && (/\s/).test(b[a]) === false && b[
                ...
                if (a < c) {
                  do {
                    ...
                    if (options.lexer_options.markup.preserveAttribute ===
                        true) {
                      lex.push(b[a])
                    } else {
                      attribute.push(b[a])
                      ...
                      if (quote === '}') {
                        if (b[a] === '{') {
                          ...
                          if (bcount === 0) {
                            ...
                            if (options.lexer_options.markup
      ...
      cheat = (function lexer_markup_tag_cheat () {
        ...
        if (ltype === 'end') {
          ...
          if (data.types[parse.count - 1] === 'singleton' && lastToken
            ...
            tname !== '/span' &&
              tname !== '/div' &&
              tname !== '/script' &&
              options.lexer_options.markup.tagMerge === true && (
              html !== 'html' || (
                html === 'html' &&
                  tname !== '/li'
              )
        ...
        if (html === 'html') {
          ...
          if (htmlsings[tname] === 'singleton') {
            if (options.attemptCorrection === true && ender.test(
              element
            ) === false) {
      ...
      if (a < c) {
        ...
        do {
          ...
          if (ext === true) {
            if (quote === '') {
              if (b[a] === '/') {
                ...
                if (quotes === 0) {
                  sparser.lexers.script(lex.join('').replace(/^(\s+)/, '')
                    .replace(/(\s+)$/, ''))
                  parse.structure[parse.structure.length - 1][1] + 1
          ...
          if (square === true && b[a] === ']') {
            a = a - 1
            ltoke = lex.join('')
            if (options.lexer_options.markup.parseSpace === false) {
          ...
          if (
            ...
            if (
              options.lexer_options.markup.parseSpace === true ||
            ...
            if (
              options.wrap > 0 &&
                options.lexer_options.markup.preserveText !== true
            ) {
