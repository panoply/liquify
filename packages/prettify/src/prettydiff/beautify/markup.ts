...
export default (function beautify_markup_init () {
  ...
  const markup = function beautify_markup (options) {
    ...
    const externalIndex = {}
    const levels = (function beautify_markup_levels () {
      const level = (prettydiff.start > 0)
        ? Array(prettydiff.start).fill(0, 0, prettydiff.start) : []
      const nextIndex = function beautify_markup_levels_next () {
        let x = a + 1
        let y = 0
        if (data.types[x] === undefined) {
          return x - 1
        }
        if (data.types[x] === 'comment' || (a < c - 1 && data
          .types[x].indexOf('attribute') > -1)) {
          do {
            if (data.types[x] === 'jsx_attribute_start') {
              y = x
              do {
                if (data.types[x] === 'jsx_attribute_end' && data
                  .begin[x] === y) {
                  break
                }
                x = x + 1
              } while (x < c)
            } else if (data.types[x] !== 'comment' && data.types[
              x].indexOf('attribute') < 0) {
              return x
            }
            x = x + 1
          } while (x < c)
      ...
      const anchorList =
            function beautify_markup_levels_anchorList () {
              let aa = a
              const stop = data.begin[a]
              // verify list is only a link list before making changes
              do {
                aa = aa - 1
                if (data.token[aa] === '</li>' && data.begin[data.begin[
                  aa]] === stop && data.token[aa - 1] ===
                  '</a>' && data.begin[aa - 1] === data.begin[aa] + 1) {
                  aa = data.begin[aa]
                } else {
                  return
                }
      ...
      const comment = function beautify_markup_levels_comment () {
        let x = a
        let test = false
        if (data.lines[a + 1] === 0 && options.forceIndent ===
                false) {
          do {
            if (data.lines[x] > 0) {
              test = true
        ...
        if (test === true) {
          const ind = (data.types[next] === 'end' || data.types[
            next] === 'template_end')
            ? indent + 1
            : indent
          do {
          ...
          if (data.types[x] === 'attribute' || data.types[x] ===
                  'template_attribute' || data.types[x] ===
                  'jsx_attribute_start') {
      ...
      const content = function beautify_markup_levels_content () {
        let ind = indent
        if (options.forceIndent === true || options
          .forceAttribute === true) {
          level.push(indent)
          return
        }
        if (next < c && (data.types[next].indexOf('end') > -1 ||
                  data.types[next].indexOf('start') > -1) && data.lines[next] > 0) {
          level.push(indent)
          ind = ind + 1
          if (data.types[a] === 'singleton' && a > 0 && data
            .types[a - 1].indexOf('attribute') > -1 &&
                  data.types[data.begin[a - 1]] === 'singleton') {
            if (data.begin[a] < 0 || (data.types[data.begin[a -
                      1]] === 'singleton' && data.begin[data
              .ender[a] - 1] !== a)) {
              level[a - 1] = indent
            } else {
              level[a - 1] = indent + 1
        ...
        } else if (data.types[a] === 'singleton' && a > 0 && data
          .types[a - 1].indexOf('attribute') > -1) {
          level[a - 1] = indent
        ...
        // * next token is template or singleton and exceeds wrap
          (options.wrap === 0 ||
                  (a < c - 2 && data.token[a].length + data.token[a + 1]
                    .length + data.token[a + 2].length + 1 >
                    options.wrap && data.types[a + 2].indexOf(
                    'attribute'
                  ) > -1) ||
                  (data.token[a].length + data.token[a + 1].length >
                    options.wrap)) &&
                (data.types[a + 1] === 'singleton' || data.types[a +
                  1] === 'template')) {
          level.push(indent)
        } else {
          count = count + 1
        ...
        if (count > options.wrap) {
          ...
          if (data.types[a] === 'content' && options
            .preserveText === false) {
            let countx = 0
            ...
            if (data.token[a].indexOf(lf) > 0) {
              count = data.token[a].length - data.token[a]
                .lastIndexOf(lf)
            }
          } else {
            do {
              ...
              if ((data.types[d] !== 'attribute' || (data.types[
                d] === 'attribute' && data.types[d +
                        1] !== 'attribute')) && data.lines[d + 1] >
                      0) {
                if (data.types[d] !== 'singleton' || (data.types[
                  d] === 'singleton' && data.types[d +
                          1] !== 'attribute')) {
                  count = data.token[a].length
                  if (data.lines[a + 1] > 0) {
                    count = count + 1
      ...
      const external = function beautify_markup_levels_external () {
        const skip = a
        do {
          if (data.lexer[a + 1] === lexer && data.begin[a + 1] <
                  skip && data.types[a + 1] !== 'start' &&
        ...
        if (
          data.lexer[next] === lexer &&
                ...
                (data.types[next] === 'end' || data.types[next] ===
                  'template_end')) {
          indent = indent - 1
        }
      }
      const attribute = function beautify_markup_levels_attribute () {
        const parent = a - 1
        const wrap = function beautify_markup_levels_attribute_wrap (
          index
        ) {
          const item = data.token[index].replace(/\s+/g, ' ')
            .split(' ')
        ...
        let plural = false
        let lev = (
          function beautify_markup_levels_attribute_level () {

            if (data.types[a].indexOf('start') > 0) {
              let x = a
              do {
                if (data.types[x].indexOf('end') > 0 && data
                  .begin[x] === a) {
                  if (x < c - 1 && data.types[x + 1].indexOf(
                    'attribute'
                  ) > -1) {
                    plural = true
                    break
            ...
            } else if (a < c - 1 && data.types[a + 1].indexOf(
              'attribute'
            ) > -1) {
              plural = true
            }
            if (data.types[next] === 'end' || data.types[
              next] === 'template_end') {
        ...
        do {
          ...
          if (data.types[a].indexOf('attribute') > 0) {
            ...
            if (data.types[a] === 'comment_attribute') {
              ...
              attStart = true
              if (a < c - 2 && data.types[a + 2].indexOf(
                'attribute'
              ) > 0) {
                level.push(-20)
                a = a + 1
        ...
        if (y > parent + 1) {
          ...
          if (
            ...
            do {
              if (data.token[y].length > options.wrap && (/\s/)
                .test(data.token[y]) === true) {
                wrap(y)
              }
              y = y - 1
              level[y] = lev
      ...
      let next = 0
      let count = 0
      let indent = (isNaN(options.indentLevel) === true) ? 0
      ...
      do {
        ...
        if (data.lexer[a] === lexer) {
          ...
          if (data.token[a].toLowerCase().indexOf('<!doctype') ===
                0) {
            level[a - 1] = indent
          }
          ...
          if (data.types[a].indexOf('attribute') > -1) {
            ...
            if (
              ...
              if (data.token[a] === '</ol>' || data.token[a] ===
                    '</ul>') {
                anchorList()
              }
            ...
            if (data.types[a] === 'script_end' && data.types[a +
                    1] === 'end') {
              ...
              if (data.lines[next] > 0 && data.types[next] ===
                    'script_start') {
              ...
              if (options.language === 'jsx' && data.token[a +
                      1] === '{') {
    ...
    return (function beautify_markup_apply () {
      const build = []
      const ind = (function beautify_markup_apply_tab () {
      ...
      // of code
      const nl = function beautify_markup_apply_nl (tabs) {
        const linesout = []
      ...
      const multiline = function beautify_markup_apply_multiline () {
        ...
        const lev = (levels[a - 1] > -1)
          ...
          : (function beautify_markup_apply_multiline_lev () {
            let bb = a - 1
            let start = (bb > -1 && data.types[bb].indexOf(
              'start'
            ) > -1)
            if (levels[a] > -1 && data.types[a] ===
                  'attribute') {
              return levels[a] + 1
            }
            do {
      ...
      const attributeEnd =
          function beautify_markup_apply_attributeEnd () {
            const parent = data.token[a]
            const regend = (/(\/|\?)?>$/)
            const end = regend.exec(parent)
            let y = a + 1
            let space = (options.selfCloseSpace === true && end !==
                null && end[0] === '/>')
              ? ' '
              : ''
            let jsx = false
            ...
            do {
              if (data.types[y] === 'jsx_attribute_end' && data.begin[
                data.begin[y]] === a) {
                jsx = false
              } else if (data.begin[y] === a) {
                if (data.types[y] === 'jsx_attribute_start') {
                  jsx = true
                } else if (data.types[y].indexOf('attribute') < 0 &&
                  jsx === false) {
                  break
                }
              } else if (jsx === false && (data.begin[y] < a || data
                .types[y].indexOf('attribute') < 0)) {
                break
              }
              y = y + 1
            } while (y < c)
      ...
      do {
        if (data.lexer[a] === lexer || prettydiff.beautify[data
          .lexer[a]] === undefined) {
          if ((data.types[a] === 'start' || data.types[a] ===
                'singleton' || data.types[a] === 'xml' || data
            .types[a] === 'sgml') && data.types[a].indexOf(
            'attribute'
          ) < 0 && a < c - 1 && data.types[a +
                1] !== undefined && data.types[a + 1].indexOf(
            'attribute'
          ) > -1) {
            attributeEnd()
          }
          if (data.token[a] !== undefined && data.token[a].indexOf(
            lf
          ) > 0 && ((data.types[a] ===
                  'content' && options.preserveText === false) || data
            .types[a] === 'comment' || data.types[
            a] === 'attribute')) {
            multiline()
          } else {
          ...
          if (externalIndex[a] === a && data.types[a] !==
              'reference') {
            build.push(data.token[a])
          } else {
            ...
            external = prettydiff.beautify[data.lexer[a]](options)
              .replace(/\s+$/, '')
            build.push(external)
            if (levels[prettydiff.iterator] > -1 && externalIndex[
              a] > a) {
