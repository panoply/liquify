...
const { log } = console;
const parserOptions = { noteKeywords: [ 'BREAKING CHANGE', 'Breaking Change' ] };
...
const commitChanges = async (cwd: string, shortName: string, version: string) => {
  if (dryRun) {
    log(ansis.yellow('Skipping Git Commit'));
    return;
  }

  log(`${ansis.blueBright('Committing')} changelog.md, package.json`);

  let params = [ 'add', cwd ];

  await execa('git', params);

  params = [ 'commit', '--m', `release: ${shortName} v${version}` ];

  await execa('git', params);
};

const getCommits = async (shortName: string) => {
  log(ansis.blueBright('Gathering Commits'));

  let params = [ 'tag', '--list', `${shortName}-v*`, '--sort', '-v:refname' ];
  const { stdout: tags } = await execa('git', params);
  const [ latestTag ] = tags.split('\n');

  log(`${ansis.blueBright('Last Release Tag')}: ${latestTag || '<none>'}`);
...
const getNewVersion = (version: string, commits: Commit[]): string | null => {
  ...
  log(ansis.blueBright('Determining New Version'));
...
const publish = async (cwd: string) => {
  ...
  if (dryRun || noPublish) {
    log(ansis.yellow('Skipping Publish'));
    return;
  }
...
const tag = async (cwd: string, shortName: string, version: string) => {
  if (dryRun || noTag) {
    log(ansis.yellow('Skipping Git Tag'));
    return;
  }

  const tagName = `${shortName}-v${version}`;

  log(`${ansis.cyan('Tagging')} ${ansis.gray(tagName)}`);
...
const updateChangelog = (commits: Commit[], cwd: string, shortName: string, version: string) => {
  ...
  log(ansis.blueBright('Gathering Changes'));

  const title = `# @liquify/${shortName} Changelog`;
  ...
  for (const commit of commits) {
    ...
    const ref = /\(#\d+\)/.test(header as string)
      ? ''
      : ` ([${hash?.substring(0, 7)}](https://github.com/liquify/commit/${hash}))`;

    const message = header?.trim()
      .replace(/\(.+\)!?:/, ':')
      .replace(/\((#(\d+))\)/, '[$1](https://github.com/liquify/pull/$2)') + ref;
  ...
  if (dryRun) {
    log(`${ansis.blueBright('New Changelog')}:\n${newLog}`);
    return;
  }

  log(`${ansis.blueBright('Updating')} changelog.md`);
...
const updatePackage = async (cwd: string, pkg: RepoPackage, version: string) => {
  if (dryRun) {
    log(ansis.yellow('Skipping package.json Update'));
    return;
  }

  log(`${ansis.blueBright('Updating')} package.json`);
...
(async () => {
  try {
    ...
    const pkg: RepoPackage = JSON.parse(readFileSync(join(cwd, 'package.json')).toString());

    if (dryRun) {
      log(`${ansis.magenta('DRY RUN')}: No files will be modified`);
    }

    log(`${ansis.cyan(`Releasing \`${packageName}\``)} from ${ansis.gray(`packages/${shortName}`)}\n`);

    const commits = await getCommits(shortName);

    if (!commits.length) {

      log(`\n${ansis.red('No Commits Found')}. Did you mean to publish ${packageName}?`);

      process.exit(1);
    }

    log(`${ansis.blueBright('Found')} ${ansis.bold(`${commits.length}`)} Commits\n`);

    const newVersion = getNewVersion(pkg.version, commits) as string;

    log(`${ansis.magenta('New Version')}: ${newVersion}\n`);
