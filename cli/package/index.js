...
var jsonMinify = _interopDefault(require('jsonminify'));
...
const encryptFile = async (name, output, state, json) => {
  ...
  await fs.writeFile(`${output}/${encryptID}.js`, `module.exports="${encryptJSON}";`);
...
const createFiles = async (output, state) => {
  ...
  // Write main entry
  await fs.writeFile(`${output}/index.js`, exported);
...
const build = async (input, output, state) => {
  ...
  const specs = await fs.readdir(input);
...
var specs = async (config, state = {
  ...
  const input = path.resolve(cwd, config.input);
  const output = path.resolve(cwd, config.output);
...
const writeSyntax = async pattern => {
  ...
  const jsonc = await fs__default.readFile(config.grammar);
...
const generateSpecs = async pattern => {
  ...
  const specs = await fs__default.readdir(config.specs);

  for (const variation of specs) {

    const content = await fs__default.readFile(`${config.specs}/${variation}`);
...
const generate = async type => {
  ...
  await fs__default.writeFile(path, JSON.stringify(dir === 'include' ? {
